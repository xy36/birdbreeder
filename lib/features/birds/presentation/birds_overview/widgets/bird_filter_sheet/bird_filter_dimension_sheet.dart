...
/// (matching the finances category filter).
///
/// Opened from the inline filter bar; returns the edited [BirdFilter] on apply.
/// The apply button shows a live count of matching birds so the effect of the
/// selection is visible before committing.
class BirdFilterDimensionSheet extends StatefulWidget {
  const BirdFilterDimensionSheet({
  ...
  final BirdFilter defaultFilter;

  /// Returns the number of birds matching a given filter.
  final int Function(BirdFilter filter) countFor;

  @override
  State<BirdFilterDimensionSheet> createState() =>
      _BirdFilterDimensionSheetState();
}

class _BirdFilterDimensionSheetState extends State<BirdFilterDimensionSheet> {
  late BirdFilter f;
  String _query = '';
  ...
  String get _title => switch (widget.dimension) {
        BirdFilterDimension.species => context.tr.common.species,
        BirdFilterDimension.sex => context.tr.common.sex.name,
        BirdFilterDimension.color => context.tr.common.color,
        BirdFilterDimension.owner => context.tr.common.owner,
        BirdFilterDimension.more => context.tr.birds.filter.more_title,
      };

  void _clearDimension() {
    setState(() {
      f = switch (widget.dimension) {
        BirdFilterDimension.species =>
          f.copyWith(speciesIds: const [], speciesName: null),
        BirdFilterDimension.color =>
          f.copyWith(colorIds: const [], colorName: null),
        BirdFilterDimension.owner =>
          f.copyWith(ownerIds: widget.defaultFilter.ownerIds),
        BirdFilterDimension.sex => f.copyWith(sexes: const []),
      ...
      builder: (context, scrollController) => Column(
        children: [
          ...
          BottomSheetFooter(
            ...
            primaryButtonLabel:
                context.tr.birds.filter.show_results(Count: widget.countFor(f)),
  ...
  List<_FilterRow> _rows(BuildContext context) {
    ...
    switch (widget.dimension) {
      case BirdFilterDimension.species:
        return [
          for (final s in r.species)
            _FilterRow(
              label: s.name ?? '-',
              selected: f.speciesIds.contains(s.id),
              onChanged: (v) => f = f.copyWith(
                speciesIds: _toggle(f.speciesIds, s.id, v),
                speciesName: null,
              ),
            ),
        ];
      case BirdFilterDimension.color:
        return [
          for (final c in r.colors)
            _FilterRow(
              ...
              onChanged: (v) => f = f.copyWith(
                colorIds: _toggle(f.colorIds, c.id, v),
                colorName: null,
              ),
            ),
        ];
      case BirdFilterDimension.owner:
        return [
          for (final c in r.contacts)
            _FilterRow(
              label: c.fullName.isNotEmpty ? c.fullName : c.number ?? '-',
              selected: f.ownerIds.contains(c.id),
              onChanged: (v) =>
                  f = f.copyWith(ownerIds: _toggle(f.ownerIds, c.id, v)),
            ),
        ];
      case BirdFilterDimension.sex:
        return [
          for (final s in Sex.values)
            _FilterRow(
              label: s.getDisplayName(context),
              selected: f.sexes.contains(s),
              secondary: s.getIcon(context, size: 20),
              onChanged: (v) => f = f.copyWith(sexes: _toggle(f.sexes, s, v)),
            ),
        ];
      case BirdFilterDimension.more:
        return [
          for (final s in SaleStatus.values)
            _FilterRow(
              label: s.getDisplayName(context),
              selected: f.saleStatus.contains(s),
              onChanged: (v) =>
                  f = f.copyWith(saleStatus: _toggle(f.saleStatus, s, v)),
            ),
          _FilterRow(
